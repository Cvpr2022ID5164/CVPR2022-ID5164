...
class ShapeNetData6D(object):
    ...
    def __init__(self, num_instances_per_item, ds_type='train', train_categ=None, test_categ=None,
        ...
        self.xmap = np.array([[j for i in range(240)] for j in range(240)])
        self.ymap = np.array([[i for i in range(240)] for j in range(240)])

        np.random.seed(config.numpy_seed)
        if ds_type != 'train':
            raise ValueError('Multiple classes dataloader can only used for training.')
        ...
        # Only consider multiple class
        self.train_categ = train_categ
        self.test_categ = test_categ
        self.num_instances_per_item = num_instances_per_item  # 50
        self.train_size = num_instances_per_item # 50
        self.test_size = num_instances_per_item # 50
        self.val_size = num_instances_per_item  # 50
        ...
        #np.random.shuffle(self.test_img_indices) # won't be changed
        #np.random.shuffle(self.val_img_indices)
        ...
        self.cls_npz_dict = {}
        self.obj_cls_dict = {}
        self.full_train_obj_list = []
        self.full_val_obj_list = []
        self.full_test_obj_list = []

        for k in self.train_categ:
            self.cls_npz_dict[k] = np.load(os.path.join(config.shapenet_npz_root, '{}.npz'.format(k)), allow_pickle=True)
            temp_list = list(self.cls_npz_dict[k].files)
            self.full_val_obj_list.extend(temp_list[-1 * config.max_val_obj:])
            temp_dict = dict.fromkeys(temp_list[-1 * config.max_val_obj:], k)
            self.obj_cls_dict.update(temp_dict)
            temp_len = min(config.max_train_obj, len(temp_list) - config.max_val_obj)
            self.full_train_obj_list.extend(temp_list[:temp_len])
            temp_dict = dict.fromkeys(temp_list[:temp_len], k)
            self.obj_cls_dict.update(temp_dict)

        for k in self.test_categ:
            self.cls_npz_dict[k] = np.load(os.path.join(config.shapenet_npz_root, '{}.npz'.format(k)),allow_pickle=True)
            temp_list = list(self.cls_npz_dict[k].files)
            temp_len = min(len(temp_list), config.max_test_obj)
            self.full_test_obj_list.extend(temp_list[:temp_len])
            temp_dict = dict.fromkeys(temp_list[:temp_len], k)
            self.obj_cls_dict.update(temp_dict)

        full_train_len = len(self.full_train_obj_list)
        full_val_len = len(self.full_val_obj_list)
        full_test_len = len(self.full_test_obj_list)

        print('Full training objects = {}, validation objects = {}, test objects = {}'.format(
            full_train_len, full_val_len,full_test_len))
        self.train_batch_per_epoch = int(self.train_size * full_train_len /
                                         (config.n_task * config.train_mini_batch_size))

        self.test_distributed_sampler = ShuffleDistributedSampler(dataset_len=full_test_len)
        indices_slice = self.test_distributed_sampler.get_indices()
        self.test_obj_list = np.asarray(self.full_test_obj_list)[indices_slice]

        self.train_distributed_sampler = ShuffleDistributedSampler(dataset_len=full_train_len)
        indices_slice = self.train_distributed_sampler.get_indices()
        self.train_obj_list = np.asarray(self.full_train_obj_list)[indices_slice]

        self.val_distributed_sampler = ShuffleDistributedSampler(dataset_len=full_val_len)
        indices_slice = self.val_distributed_sampler.get_indices()
        self.val_obj_list = np.asarray(self.full_val_obj_list)[indices_slice]
        ...
        self.estimated_it = {
            ...
            'train': self.iter_count['train'] *  len(self.train_obj_list) // config.n_task,
            'val': self.iter_count['val'] * len(self.val_obj_list) // config.n_task
        }
        ...
        print('-----> {}_dataset: train {} objects per GPU, estimated iteration = {}'.format(
            ds_type, len(self.train_obj_list), self.train_batch_per_epoch))
    ...
    def get_batch(self, source, tasks_per_batch):
        ...
        for i in range(tasks_per_batch):
            ...
            try:
                if task_dict['cls_name'].shape[0] < mini_batch_size:
                    batch_dict = dict(invalid_it=True)
                    print('111')
                    return batch_dict
    ...
    def get_item_batch_wise(self, obj_name, batch_indices):
        ...
        Returns: dictionary
        '''
        cls_name = self.obj_cls_dict[obj_name]
        obj_arr = self.cls_npz_dict[cls_name][obj_name]
    ...
    def get_item(self, obj_name, item_arr):
        ...
        if len(choose) < 200:
            print('Invalid len: {}, cls_name: {}, obj_name: {}'.format(len(choose),cls_name, obj_name))
            # return None
        ...
        kp3ds, ctr3ds, cls_ids, kp_targ_ofst, ctr_targ_ofst, kps_ctr = self.get_pose_gt_info(
            cld, labels_pt, RT, obj_name,cls_name)
...
def main():
    ...
    ds_type = 'train'
    cls_name = 'car'
    ds = ShapeNetData6D(num_instances_per_item=4, ds_type=ds_type, train_categ=['train', 'airplane'],
                        test_categ=['piano'], DEBUG=False)
    ...
    while True:
        data = ds.get_batch(source='train', tasks_per_batch=2)
        if data is None:
            break
