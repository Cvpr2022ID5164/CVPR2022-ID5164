...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck", max_sample=None, DEBUG=False):
        # bounding box size
        ...
        if dataset_name == 'train':
            ...
            real_img_pth = os.path.join(
                self.cls_root, "test.txt" # experiment
                #"train.txt" 
            )
            ...
            if max_sample is None:
                ...
                self.minibatch_per_epoch = max_sample // self.config.mini_batch_size
        else:
            self.add_noise = False
            tst_img_pth = os.path.join(self.cls_root, "train.txt" #experiment
                                       #"test.txt"
                                      )
    ...
    def get_item(self, item_name):
        ...
        msk_dp = dpt_mm > 1e-6
        non_background = msk_dp.flatten().nonzero()[0].astype(np.uint32)
        if len(non_background) < 400:
            return None
        ...
        # get tight bounding box of the object
        img_height, img_width = labels.shape
        obj_rgb = labels.flatten().nonzero()[0].astype(np.uint32)
        # print('#Obj idx = ',obj_rgb.shape[0])
        min_i = img_height
        max_i = 0
        min_j = img_width
        max_j = 0
        ...
        for idx in obj_rgb:
            ...
            min_i = min(min_i, idx_i)
            max_i = max(max_i, idx_i)

            min_j = min(min_j, idx_j)
            max_j = max(max_j, idx_j)

        ctr_i = (min_i + max_i) // 2
        ctr_j = (min_j + max_j) // 2

        # adjust center to avoid larger bounding box out of boundary
        if ctr_i < self.half_height:
            ctr_i_new = self.half_height
        elif ctr_i >= (img_height - self.half_height):
            ctr_i_new = img_height - self.half_height - 1
        else:
            ctr_i_new = ctr_i

        if ctr_j < self.half_width:
            ctr_j_new = self.half_width
        elif ctr_j >= (img_width - self.half_width):
            ctr_j_new = img_width - self.half_width - 1
        else:
            ctr_j_new = ctr_j

        # calculate large bounding box corners
        bd_left_top_i = ctr_i_new - self.half_height  # crop bounding box, size 240*240
        bd_left_top_j = ctr_j_new - self.half_width
        bd_right_bot_i = ctr_i_new + self.half_height
        bd_right_bot_j = ctr_j_new + self.half_width

        # get local coordinate (in cropped image)
        ctr_i_crop = ctr_i - bd_left_top_i  # center coordinate in cropped image
        ctr_j_crop = ctr_j - bd_left_top_j
        obj_bd_width = max_j - min_j  # object tight bounding box width
        obj_bd_height = max_i - min_i
        obj_bd_left_top_i = ctr_i_crop - obj_bd_height // 2
        obj_bd_left_top_j = ctr_j_crop - obj_bd_width // 2

        # crop values into smaller image
        rgb = rgb[bd_left_top_i:bd_left_top_i + self.height,
              bd_left_top_j:bd_left_top_j + self.width, :]
        labels = labels[bd_left_top_i:bd_left_top_i + self.height,
                 bd_left_top_j:bd_left_top_j + self.width]
        rgb_labels = rgb_labels[bd_left_top_i:bd_left_top_i + self.height,
                     bd_left_top_j:bd_left_top_j + self.width]
        cropped_nrm_map = nrm_map[bd_left_top_i:bd_left_top_i + self.height,
                          bd_left_top_j:bd_left_top_j + self.width]
        cropped_xyz = dpt_xyz[bd_left_top_i:bd_left_top_i + self.height,
                      bd_left_top_j:bd_left_top_j + self.width]
        obj_rgb = labels.flatten().nonzero()[0].astype(np.uint32)
        if self.DEBUG:
            print('#Obj rgb = ', obj_rgb.shape[0])
        obj_idx_lst = obj_rgb.copy()

        # sample points on the object if possibile
        '''  
        if len(obj_idx_lst) >= self.config.n_sample_points:
            c_mask = np.zeros(len(obj_idx_lst), dtype=int)
            c_mask[:self.config.n_sample_points] = 1
            np.random.shuffle(c_mask)
            choose = obj_idx_lst[c_mask.nonzero()]
        else:
            all_idx = np.arange(self.height*self.height)
            choose_2_1 = obj_idx_lst
            choose_2_2 = np.delete(all_idx, choose_2_1)
            c_mask = np.zeros(len(choose_2_2), dtype=int)
            c_mask[:self.config.n_sample_points-len(choose_2_1)] = 1
            np.random.shuffle(c_mask)
            choose_2_2 = choose_2_2[c_mask.nonzero()]
            choose_2 = np.append(choose_2_1,choose_2_2,0)
            choose = np.sort(choose_2)
        '''

        choose_on_obj = 1200
        if len(obj_idx_lst) < choose_on_obj:
            choose_2_1 = obj_idx_lst
        else:
            c_mask = np.zeros(len(obj_idx_lst), dtype=int)
            c_mask[:choose_on_obj] = 1
            np.random.shuffle(c_mask)
            choose_2_1 = obj_idx_lst[c_mask.nonzero()]

        all_idx = np.arange(self.height * self.height)
        choose_2_2 = np.delete(all_idx, obj_idx_lst)
        c_mask = np.zeros(len(choose_2_2), dtype=int)
        c_mask[:self.config.n_sample_points - len(choose_2_1)] = 1
        np.random.shuffle(c_mask)
        choose_2_2 = choose_2_2[c_mask.nonzero()]
        ...
        cld = cropped_xyz.reshape(-1, 3)[choose, :]
        rgb_pt = rgb.reshape(-1, 3)[choose, :].astype(np.float32)
        nrm_pt = cropped_nrm_map.reshape(-1, 3)[choose, :]
        ...
        dpt_6c = np.concatenate((cropped_xyz, cropped_nrm_map[:, :, :3]), axis=2).transpose(2, 0, 1) #chw
        rgb = np.transpose(rgb, (2, 0, 1))  # hwc2chw

        xyz_lst = [cropped_xyz.transpose(2, 0, 1)]  # c, h, w
        #msk_lst = [cropped_xyz[2, :, :] > 1e-8]
        '''
        if True:
            show_rgb = original_rgb[...,::-1].copy()
            pcld = cropped_xyz.reshape(-1,3)
            show_rgb = cv2.rectangle(show_rgb, (bd_left_top_j, bd_left_top_i),
                                     (bd_right_bot_j, bd_right_bot_i), (255, 0, 0), 3)
            p2ds = self.bs_utils.project_p3d(cld, cam_scale, K)
            #show_rgb = self.bs_utils.paste_p2ds(show_rgb.copy(), p2ds, (0, 0, 255))
            #show_rgb =  self.bs_utils.draw_p2ds(show_rgb, p2ds, color=(0, 0, 255))
            show_rgb = self.bs_utils.paste_p2ds(show_rgb.copy(), p2ds, (0, 0, 255))
            imshow("test point clod image",show_rgb)
            imshow("Original", original_rgb.copy()[:, :, ::-1])
            waitKey(0)
        '''
        for i in range(3):
            scale = pow(2, i+1)
        ...
        item_dict = dict(
            cls_id=np.array([self.cls_id],dtype=np.uint8),
